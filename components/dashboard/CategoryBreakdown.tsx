...
import { useGetCategoryBreakdown } from "@/api/generated/dashboard-controller/dashboard-controller";
      ...
      <div className="p-5 rounded-2xl border border-zinc-800 bg-zinc-900 shadow-xl space-y-4">
        <div className="flex items-center justify-between border-b border-zinc-800/80 pb-3">
          <div>
            ...
            <p className="text-xs text-zinc-400">Loading breakdown...</p>
          </div>
        </div>
    ...
    <div className="p-5 rounded-2xl border border-zinc-800 bg-zinc-900 shadow-xl space-y-4">
      ...
      <div className="space-y-3.5">
        {categories.map((cat, idx) => {
          ...
          const meta = categoryMetadataMap[catName];
          const Icon = catName !== 'Uncategorized' && meta ? meta.icon : TriangleAlert;
          const iconColor = catName !== 'Uncategorized' && meta ? meta.color : 'bg-amber-900/20 text-amber-600';
          const percentage = cat.percentage ?? 0;
          const amount = cat.totalAmount ?? 0;

          return (
            <div key={cat.categoryName || idx} className="space-y-1.5">
              <div className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  <div className={`p-1.5 rounded-lg border ${iconColor}`}>
                    <Icon className={`h-3.5 w-3.5 ${iconColor}`} />
                  </div>
                  <span className="font-semibold text-zinc-200">{catName}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-zinc-400 text-[11px]">{percentage.toFixed(2)}%</span>
                  <span className="font-bold text-white">{formatCurrency(amount)}</span>
                </div>
              </div>

              <div className="h-1.5 w-full bg-zinc-800 rounded-full overflow-hidden">
                <div className={`h-full ${iconColor.split(' ')[0].replace('900/20', '500')} rounded-full transition-all`} style={{ width: `${percentage}%` }} />
              </div>
            </div>
          );
        })}
